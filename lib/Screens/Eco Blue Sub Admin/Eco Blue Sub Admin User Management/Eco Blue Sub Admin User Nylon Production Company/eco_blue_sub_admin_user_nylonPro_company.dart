...
class EcoBlueSubUserNylonProCompany extends StatefulWidget {
  const EcoBlueSubUserNylonProCompany({super.key});

  @override
  State<EcoBlueSubUserNylonProCompany> createState() => _EcoBlueSubUserNylonProCompanyState();
}

class _EcoBlueSubUserNylonProCompanyState extends State<EcoBlueSubUserNylonProCompany> {
  ...
  Future<void> _fetchData() async {
    int currentDataLength = nylonProductionCompanyController?.getRevenueShareListModel?.data.length??0;

    if (!isLoadingMoreData && nylonProductionCompanyController?.getRevenueShareListModel?.data.length != nylonProductionCompanyController?.getRevenueShareListModel?.pagination?.totalItems ) {
      ...
      nylonProductionCompanyController?.getRevenueShareListApi(context, "1", '$limit','',nylonProductionCompanyController?.nylonModel?.data?.revenueModelId).then(
              (value){
            debugPrint(nylonProductionCompanyController?.getRevenueShareListModel?.data.length.toString());
            setState(() {
  ...
  NylonProductionCompanyController? nylonProductionCompanyController;
  ...
  Widget build(BuildContext context) {
    return GetBuilder(
      ...
      builder: (controller) => Padding(
        ...
        child:controller.nylonModel == null?
        Center(child: myShimmer())
            :controller.nylonModel?.status == false?  Center(child: Text("Nylon production company not found")) : Column(
          children: [
            Card(
              elevation: 1,
                ...
                children: [
                  Flexible(
                    child: CustomSearchTextField(
                      onChanged: (value){
                        print("this is si ${controller.nylonModel?.data?.revenueModelId}");
                        controller.getRevenueShareListApi(context, "1", "10", value, controller.nylonModel?.data?.revenueModelId);
                      },
