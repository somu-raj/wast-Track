...
class EcoBlueSubUserEnforcementTeam extends StatefulWidget {
  const EcoBlueSubUserEnforcementTeam({super.key});

  @override
  State<EcoBlueSubUserEnforcementTeam> createState() => _EcoBlueSubUserEnforcementTeamState();
}

class _EcoBlueSubUserEnforcementTeamState extends State<EcoBlueSubUserEnforcementTeam> {
  bool isButtonSelected = false;
  final enforcementTeamController1 = Get.put(EnforcementController());
  ...
  Future<void> _fetchData() async {
    int currentDataLength = enforcementController.getRevenueShareListModel?.data.length??0;

    if (!isLoadingMoreData && enforcementController.getRevenueShareListModel?.data.length != enforcementController.getRevenueShareListModel?.pagination?.totalItems ) {
      ...
      enforcementController.getRevenueShareListApi(context, "1", '$limit','',enforcementController.enforcementModel?.data?.revenueModelId).then(
              (value){
            debugPrint(enforcementController.getRevenueShareListModel?.data.length.toString());
            setState(() {
  ...
  Widget build(BuildContext context) {
    return GetBuilder(
      ...
      builder: (controller)=> Padding(
        ...
        child: controller.enforcementModel == null?
        Center(child: myShimmer())
            :controller.enforcementModel?.status == false?  Text("Enforcement team not found") : Column(
          children: [
            Card(
              elevation: 1,
                ...
                children: [
                  Flexible(
                    child: CustomSearchTextField(
                      onChanged: (v){
                        enforcementController.getRevenueShareListApi(context, "1","10","", enforcementController.enforcementModel?.data?.revenueModelId);
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Obx(
                        ...
                        return Column(
                          children: [
                            Visibility(
                              visible: enforcementTeamController1
                            ...
                            Visibility(
                              visible: enforcementTeamController1.isContainerVisible.isFalse,
                              child: Stack(
                                clipBehavior: Clip.none,
                                children: [
                                  Container(
                                  ...
                                  Positioned(
                                    ...
                                    child: Container(
                                      ...
                                      child: GestureDetector(
                                        onTap: () {
                                          enforcementTeamController1.ContainerHide();
                                        },
                                        child: Icon(
